...
class Robot:
    ...
    def __init__ (self):
        ...
        self.cmdLastTime = rospy.get_rostime()
    ...
    def teleop_input(self):
        ...
        if(user_input == 8 or user_input == 2 or user_input == 4 or user_input == 6 or user_input == 5):
            if(user_input == 8):
                self.command_twist.linear.x = 0.20
            ...
            if(user_input == 4):
                ...
                self.command_twist.angular.z = -0.30
    ...
    def callback_cmd_vel(self, msg):
        ...
        #Transform unicycle value into rad per sec wheel
        ...
        #Cross product radiant per second to 0-255
        self.v_r = abs(int((255 * radian_per_sec_r) / 7.84))
        self.v_l = abs(int((255 * radian_per_sec_l) / 7.84))

        #Chech value for min and max speed, replace them if necessary
        self.v_r = max(min(self.v_r, self.speed_max), self.speed_min)
        self.v_l = max(min(self.v_l, self.speed_max), self.speed_min)
        ...
        if(msg.linear.x == 0):
            if(msg.angular.z < 0):
                print("msg.angular.z < 0")
                self.v_r = max(self.v_r, self.v_l)
                self.v_l = max(self.v_r, self.v_l) + 1000
            else:
                print("msg.angular.z > 0")
                self.v_l = max(self.v_r, self.v_l)
                self.v_r = max(self.v_r, self.v_l) + 1000

        print("before check backward")
        print("self.v_r is: " + str(self.v_r))
        print("self.v_l is: " + str(self.v_l))

        #Check for backward motion
        if (msg.linear.x < 0):
            print("inside if going backward")
            self.v_l += 1000
            self.v_r += 1000
        ...
        self.publish_cmd_vel_stop(2)

    def publish_cmd_vel(self):
        self.command_twist.linear.x = 0.0
        self.command_twist.angular.z = 1.0
        self.cmd_vel_pub.publish(self.command_twist)
    ...
    def publish_cmd_vel_stop(self, wait_time):
        ...
        self.v_r_pub.publish(self.v_r)
        self.v_l_pub.publish(self.v_l)
        #elf.cmd_vel_pub.publish(self.command_twist)
...
if __name__ == '__main__':
    rospy.init_node('billy_rosserial_pc')
    messia = Robot()
    messia.publish_cmd_vel_stop(3)
